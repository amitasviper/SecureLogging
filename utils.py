...
class BloomFilter(object):
    def __init__(self, capacity=500000, bloom_dict=None, error_rate=0.01, ip=None):
        ...
        if bloom_dict == None:
            ...
            printMessage(3, "Item Capacity: %d, Error Rate: %f, Bits Count: %d, Hash Count: %d" % (
                self.capacity, self.error_rate, self.bits_count, self.hash_count))
...
def EncryptData(dict_data, encrypting_key, dict_keys=None):
    ...
    data = data[:-1]
    return encryptData(data, encrypting_key)


def encryptData(str_data, encrypting_key):
...
def DecryptData(str_data, decrypting_key):
    decrypted = decryptData(str_data, decrypting_key)
...
def GeneratePPL():
    global connection
    accumulators_cursor = connection.GetAllTodaysAccumulators()
    ...
    for i in range(accumulators_cursor.count()):
        ...
        private_key = GetRSAKey("CSP", "private")
        ppl_dict = generateSignature(private_key, data)
...
def getKeyPath(agency_name, ktype):
    pwd = os.getcwd()
    dir_key = os.path.join(pwd, 'keys')
    apath = None
    if agency_name == "LEA":
...
def getRSAKey(agency_name, ktype=None):
    ...
    csp_private_key = getKeyPath("CSP", "private")
...
def VerifySignature(public_key_str, actual_data, signature):
    public_key = GetRSAKey("CSP", "public")
...
def bloomNDatabaseConnection():
    ...
    bloom_dict['ip'] = ip
    global connection
    current = connection.GetCurrentAccumulator(ip)
    if current == None:
        connection.UpdateAccumulator(bloom_dict)
    else:
