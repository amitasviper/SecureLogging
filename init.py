...
import utils, dbhelper, unicodedata
...
def download(filename):
    filepath = None
    if filename == "LEA":
        filepath = utils.getKeyPath("LEA", "public")
    else:
        filepath = utils.getKeyPath("CSP", "public")
    content = ""
...
def GenererateLogsOnUserRequest():
    ...
    for i in range(200):
        ...
        encrypted_log_entry = {}
        encrypted_log_entry['encrypted_log'] = utils.EncryptData(log_entry, public_key,
                                                                 dict_keys=['from_ip', 'to_ip', 'port', 'time'])
        ...
        current = connection.GetCurrentAccumulator(encrypted_log_entry['from_ip'])
        ...
        connection.UpdateAccumulator(bloom_filter.Serialise())
...
def channel_keys(c_req):
    ...
    cursor = connection.FetchAllPPl()
    response = []
    public = utils.GetRSAKey("LEA", "public")
    res = {'AgencyName': 'LEA', 'keyval': public.exportKey()}
    response.append(res)
    public = utils.GetRSAKey("CSP", "public")
...
def channel_log_verify(data):
    ...
    private_key = utils.GetRSAKey("LEA", "private")
    try:
